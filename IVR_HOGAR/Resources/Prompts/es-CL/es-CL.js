 ...
 */
var promptBaseUrl = "../Resources/Prompts/es-CL/";
 ...
 */
function esCLPlayBuiltinType(value, type, promptUrl, format)
{
...
function datePrompts(value, format)
{
    ...
    if (yearKnown && monthKnown && dayKnown) {
		...
		if (speakDay) {
            promptsArray = promptsArray.concat(ordinalPrompts(day));
			//promptsArray = promptsArray.concat(promptBaseUrl + "miscellaneous/de" + audioFileExtension);
        }
        if (speakMonth) {
            promptsArray = promptsArray.concat(monthPrompts(month));
...
function cardinalPrompts(number)
{
	...
	if (number == 1) {
        return new Array(promptBaseUrl + "cardinals/001" + audioFileExtension);
    }
    ...
    if (number == 1000){
    	...
    	while (number > 0) {
            ...
            if (endDigits != 0) {
                promptsArray = threeDigitsPrompts(endDigits).concat(magnitudePrompts(magnitude, endDigits), promptsArray);
            }
...
function threeDigitsPrompts(number)
{
    ...
    if (tensAndOnes > 0) {
        if (tensAndOnes < 10) {
        	...
        	if ((tensAndOnes % 10 == 1) && (tensAndOnes != 11)) {
                promptsArray.push(promptBaseUrl + "cardinals/0" + tensAndOnes + "o"+ audioFileExtension);
            }
            else {
...
function ordinalThreeDigitsPrompts(number)
{
    ...
    if (hundreds > 0) {
        if (tensAndOnes > 0) {
            promptsArray.push(promptBaseUrl + "ordinals/" + hundreds + "00" + audioFileExtension);
        } else {
            promptsArray.push(promptBaseUrl + "ordinals/00" + hundreds + "" + audioFileExtension);
    ...
    if (tensAndOnes > 0) {
        if (tensAndOnes < 10) {
        	if (tensAndOnes == 1){
        		promptsArray.push(promptBaseUrl + "ordinals/un" + audioFileExtension);
        	}else{
 ...
 * @return An array of URL strings.
 */
function magnitudePrompts(magnitude, number)
{
    switch (magnitude) {
        case 0:
            return new Array();
